---
title: "svm01xht: Support Vector Machine (SVM) for Binary Classification with Hypothesis Testing Utilities"
...
vignette: >
  %\VignetteIndexEntry{svm01xht: Support Vector Machine (SVM) for Binary Classification with Hypothesis Testing Utilities}
...
In this example dataset, the number of features is small. However, in other datasets, number of features can be very large that feature selection should be done for efficiency. This package has `select_correlated_features` function to choose top "n" correlated features if "n" is given, or all features with absolute correlation to the label above some "min" value between 0 and 1.
...
This package was developed as a short learning project to fulfill course requirements. The goal was to provide hands-on experience in R package development, while exploring data science concepts. Therefore, it relies minimally on external libraries and contains areas that remain incomplete.
